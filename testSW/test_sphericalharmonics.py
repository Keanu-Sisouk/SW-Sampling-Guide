...
from utils import *
from sliced_wasserstein import SHCV, SW_CV
import time
...
# for d in dim_list:
    ...
    for p in range(100):
        ...
        for i in range(len(N_list)):
            ...
            t0 = time.time()
            SW = SW_CV(X, Y, seed,L=N_list[i], p=2, Phi=Phi)
            t1 = time.time()
            total = t1 - t0
        ...
        np.save("new_timers_sphereharmonics_SWIncluded_in_"+str(d)+"D.npy", np.array(timers_list))
    ...
    np.save("new_errors_sphereharmonics_SWIncluded_in_"+str(d)+"D.npy", np.array(mean_error_list))
