...
# for d in dim_list:
    ...
    for i in range(len(N_list)):
        ...
        SW, total = SW_QMC(X,Y, L = N_list[i], seq="halton")
        # SW = MY_SW_2(X,Y,np.array(new_points))
        errors_list.append(abs(SW - ground_truth))
        timers_list.append(total)
    
    np.save("timers_halton_leluc_in_"+str(d)+"D.npy", np.array(timers_list))
    np.save("errors_halton_leluc_in_"+str(d)+"D.npy", np.array(errors_list))
